...
<body>
    ...
    <g:each in="${devices}" var="device">
        <div style="position:absolute; top:${device.value.y}px; left:${device.value.x}px;">
            <img  src="${createLink(controller:'relImages', action:'view', params:[code:code, dev:device.key, path:device.value.path])}" alt="No images for ${code}_${device.key}" />
        </div>
    </g:each>
</body>
</html>
